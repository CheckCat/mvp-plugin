...
**Ни одно поле тут не стоимость.** `delta_tokens` — взгляд контроллера, субагентов не видит (на vireo занизил в **8.4×**). `dispatches` — не прокси (корреляция −0.04), а **форма прогона**: 7 — чистый путь, 10 — лестница валидации (до 2026-08-26: 5 и 8, ревьюера опрашивали раз, а не трижды). В $ не переводи.
