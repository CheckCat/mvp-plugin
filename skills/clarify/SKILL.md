...
**Модель.** Этот скилл целиком идёт в главном цикле сессии — своей модели у него нет, он наследует модель оператора. Здесь принимаются решения, которые build уже не пересматривает: на glotok формула из §14 попала дословно в заголовок задачи, и обе стороны сплит-эксперимента её просто транскрибировали. Если сессия не на сильнейшей доступной модели — **скажи это оператору первой строкой и предложи перезапустить сессию**, прежде чем формулировать находки. Дешёвая модель здесь экономит центы и стоит переписанной архитектуры.
