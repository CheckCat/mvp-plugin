...
**Модель.** Скилл идёт в главном цикле и наследует модель сессии. Здесь принимаются решения, которые build уже не пересматривает. Сессия не на сильнейшей доступной модели — скажи оператору первой строкой и предложи перезапуск, прежде чем формулировать находки.
...
## Шаг 3.5 — два прохода, которых Шаг 3 не делает

Шаг 3 ищет дыры **в тексте**. Все три реальных дефекта glotok пришли не
оттуда: текст был полон, но решения конфликтовали между собой, и никто не
спросил «а если это случится дважды». Пиши находки в ту же очередь.

**3.5a — противоречия.** Сверь решения попарно: одно снимает ограничение, на
которое опирается другое. Пример: одно оставило `end_time` без границ, второе
привязало период к календарному дню — вместе `end_time = 00:00` принимается и
не работает никогда. Такая находка всегда `critical`.

**3.5b — жизненный цикл.** Для каждой сущности со статусом и каждого действия
пользователя: что если **повторить**? **порядок нарушен**? **одновременно**?
**оборвалось на середине**? Спека, которая не отвечает, — дыра.
