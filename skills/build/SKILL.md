...
| `all-done` | ТОЛЬКО `detail` (обычно пусто) — `results`/`tasks_done` в этом halt НЕ возвращаются, даже если этот же запуск успел закоммитить задачи до того как упёрся в `all-done` на очередной итерации цикла; факт смотри в `ledger.md`/`git log`, не в payload | `${CLAUDE_PLUGIN_ROOT}/lib/state.sh set phase done`, покажи хвост `ledger.md`. **NEXT: Use mvp:retro**. |
...
| `dirty-tree` | `detail` здесь ненадёжен: `plan-io.mjs next` кладёт список в поле `files`, а `workflow.mjs` при пробросе халта читает только `adv.data.detail` — фактически придёт `detail: undefined` (известный разрыв в уже сданном `workflow.mjs`, задокументирован как есть, не мой файл на этой задаче) | Не полагайся на payload — выполни сам `git status --porcelain -- . ':!.claude/state'`, покажи файлы оператору, предложи `git checkout -- <files>` (сброс) или ручной коммит вне пайплайна, затем перезапусти. |
...
| Соблазн | Почему нет |
|---|---|
| «План почти валиден, поправлю поле на лету» | так v1 терял `service` и валил рабочий код — правки `plan.json` только через `plan-io.mjs` |
| «Ревью можно скипнуть, молекула тривиальная» | 7/16 тривиальных молекул baseline содержали реальные баги |
| «git add -A, файлов много» | `finalize.sh` внутри workflow стейджит explicit-списком, всегда — тот же принцип держи и для своих собственных ручных append'ов (ledger/decisions.log — точечно, не массово) |

## Red flags

«Перепишу этот JSON сам» (`plan.json`/Task-строки `ledger.md` — только скрипт). «Вызову `git commit` напрямую» (код+state коммитит `finalize.sh build-task` изнутри workflow — ты `git commit` вообще не вызываешь). «Запущу второй workflow параллельно» (один working tree — гонка коммитов). «Использую `resumeFromRunId`» (мёртвая фича здесь).
