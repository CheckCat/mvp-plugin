...
| `stop-and-ask` | `task_id`, `detail` = причина `park()` (implementer BLOCKED/NEEDS_CONTEXT текст, либо validate/review-лестница исчерпана, либо **неполный ревью-пакет**: `review package is incomplete — ... truncated` / `reviewer could not verify ...`, либо **роль не продиспатчилась**: `agentType "<role>" did not dispatch`) † | Прочитай статус `task_id` в `.mvp/plan.json` (уже `failed`, дерево уже чистое — `park()` сам делает `git checkout`+unstage) и `.mvp/blockers.md` (пишет диспатченный агент по контракту `_common.md`, не `workflow.mjs`). `AskUserQuestion` с этим контекстом. Решение запиши в `.mvp/decisions.log` — Write/Edit append строкой `[task_id] решение — обоснование` (это НЕ pipeline-state, а журнал оператора; пишет сама сессия, не скрипт). Перезапусти ту же задачу явным `task_id`. **`agentType did not dispatch`** — не ruling, а перезапуск СЕССИИ: есть `.claude/agents/<role>.md` → перезапусти сессию (роли регистрируются при её старте); нет → роль не собрана, перезапуск не поможет.<br><br>**Блокер называет работу вне границы задачи?** Заведи отдельную: `plan-io.mjs add-task --json '{...}'` — присвоит id, поставит `pending`, запишет только если план остаётся валидным. Не правь plan.json руками и не бросай блокер в `blockers.md`: так на vireo потерялся циклический импорт, ронявший два деплой-юнита. |
