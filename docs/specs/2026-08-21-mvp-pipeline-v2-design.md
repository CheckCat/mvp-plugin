...
**A.8. Проверка `declared` — односторонняя.** `validate-task.sh` больше не
сообщает `undeclared-files`. Остаётся только `missing-declared`: план обещал
артефакт, а его нет. Обоснование — базовая частота: старая двусторонняя форма
срабатывала на 35 задачах из 36, то есть не несла информации; оставшаяся
половина срабатывает на 20 из 36 и каждое срабатывание означает реальное
расхождение плана с реальностью. Нарушение по-прежнему non-blocking (concern),
но теперь concern доезжает до ledger (A.4), а не теряется.

Поле `files` **намеренно не переименовано** в `must_create`. Рассматривалось и
отклонено: `files` читает не только этот гейт — `validate-plan.py`
(`check_frontend_backend_dep`) использует список путей как эвристику «фронтовая
задача трогает api-клиент, значит нужен backend-предок в depends_on». Указание
планировщику сокращать список до «только то, в чём уверен» ослабило бы эту
эвристику ради косметики: смысловую часть — прекратить считать нарушением
файлы, которых план не называл, — даёт сама односторонность, без churn'а схемы
и без несовместимости с уже написанными планами.

**A.9. Неразбираемый ответ ревьюера — один переспрос, потом парковка.** Если
ревьюер вернул `request-changes`, а `FINDINGS` не распарсился, `workflow.mjs`
переспрашивает его один раз (`reviewer-retry-<id>`), требуя строгий однострочный
JSON и короткие `summary`/`quote`. Повторная неудача паркует, как раньше.

Основание — живой отказ на задаче 037: ревьюер выдал длинный массив находок с
битой записью (`"summary":"…", "}`), парсинг упал, задача была припаркована и
`park()` откатил границу — **371 000 токенов уже готовой работы implementer'а
выброшены из-за оговорки в самом гейте**, а не из-за состояния кода. Парковка —
правильный ответ на негодный код и очень плохой ответ на сбой транскрипции.
Переспрос стоит одного диспатча ревьюера против полного переигрывания задачи.

Переспрошенный ответ судится на общих основаниях: `approve` и `PATCHES` из него
принимаются наравне с первым ответом — ревьюеру, которого просят изложить
вердикт заново, не запрещено прийти к «претензий нет». Факт переспроса уходит
в ledger как concern, чтобы молчаливого «само починилось» не было.

**A.10. `plan-io.mjs add-task` — план перестал быть замороженным.** Один
глагол: добавить ОДНУ задачу в конец плана, с присвоением следующего id,
принудительным `pending` и записью только если результирующий план проходит ту
же валидацию, что `validate` (общая функция, не вторая реализация «легальности»).
Редактирование и удаление существующих задач намеренно не поддержаны — done-задачи
это история с реальными коммитами.

Основание: за прогон vireo пробел ударил дважды, второй раз с ущербом. Devops-агенты
задач 048 и 052 нашли циклический импорт, ронявший `worker` и `beat` в restart-loop,
корректно отказались править прикладной код вне своей границы и написали блокер — и
находка осталась лежать, потому что превратить её в задачу было нечем. Чинили руками,
вне пайплайна, уже после того как DAG отчитался `all-done`.

**A.11. Слой разбора покрыт тестами.** `workflow.mjs` — единственный компонент,
разбирающий недоверенный текст (через него идёт каждый ответ каждого субагента), и
до 2026-08-25 у него было ноль тестов при тринадцати сюитах на детерминированных
шелл-скриптах. Оба production-сбоя пайплайна случились именно там. Харнесс
`tests/lib/workflow-parsers.mjs` извлекает настоящие объявления из настоящего файла
(копия дрейфовала бы и не проверяла ничего) и фиксирует их на реальных упавших
payload'ах. Найдено сразу три бага: `parseStatus` не различал `DONE_WITH_CONCERNS`
(21 ответ на 20 задачах прочитан как `DONE`, все авторские concern'ы потеряны),
`parseCannotVerify` открывался наружу на префиксе, `extractField` при пустом поле
захватывал следующую строку.

**A.12. Бюджеты размера SKILL.md — тест, а не пожелание.** Числа берутся из §«Размер»
этой спеки, не из текущего веса файлов. Правка retro превысила потолок на 23 %, и
заметил это только ручной `wc -c`.

**A.13. `dispatches` — не прокси стоимости (измерено, гипотеза отвергнута).**
Вводя поле (A.6), я утверждал, что число диспатчей масштабируется с реальным
расходом, потому что каждый несёт floor ~30 200 токенов. Замер на 19 задачах
новой версии это **опроверг**: корреляция `dispatches` с реальными токенами
+0.28, с $-взвешенной стоимостью **−0.04**. Причина: у 14 задач из 19 ровно 5
диспатчей — переменная почти без дисперсии, — тогда как стоимость гуляет в 3.5
раза по токенам и в 22 раза по деньгам. Пол диспатча перестал быть главной
статьёй после релей-диеты; всё остальное определяют работа implementer'а и тир
модели, чего счётчик вызовов не видит.

Поле остаётся, но с честной семантикой: это **форма прогона**, не цена. `5` —
чистый путь, `8` — сработала лестница валидации. Как сигнал трения оно
полезно, как бюджетная величина — нет. `delta_tokens` в той же выборке дал
+0.33/+0.18, то есть не лучше. Вывод для оператора: телеметрия пайплайна не
отвечает на вопрос «сколько стоила задача», и притворяться, что отвечает,
хуже, чем сказать это прямо.

**A.14. `phase=done` ставит скрипт, а не SKILL.** На халте `all-done`
`workflow.mjs` сам вызывает `state.sh set phase done` и возвращает
`phase_set`. Раньше это была инструкция контроллеру в build/SKILL.md — и на
прогоне vireo она не выполнилась ни разу: контроллер дёргал Workflow напрямую,
фаза так и осталась `plan-done` после всех 55 задач, а гейт `mvp:retro`
(`phase != done` → Stop&Ask) закономерно отказался стартовать. Ровно та же
форма отказа, что с concern'ами (A.4): запись состояния, зависящая от того,
вспомнит ли LLM, рано или поздно не происходит. Суждения в этом переходе нет —
все задачи плана done, — значит место ему в скрипте. Неудача записи не делает
законченный план ошибкой, но и не проходит молча: предупреждение уезжает в
`detail`.

**A.15. Отчёт retro пишется в ПРОЕКТ, не в кэш плагина.** Шаг 4 предписывал
`${CLAUDE_PLUGIN_ROOT}/docs/observations/…`, но `CLAUDE_PLUGIN_ROOT` — это
`~/.claude/plugins/cache/…`, копия установки, которую стирает следующий
`claude plugin install`. Оговорка «если read-only — предупреди оператора» не
срабатывала: кэш writable, файл записался бы молча и был бы потерян при
ближайшей переустановке. Новый путь — `.claude/state/retro-<date>.md` в
проекте: он переживает переустановки и коммитится вместе с проектом. Перенос
кандидатов в репозиторий плагина остаётся ручным (Iron Law не меняется).

**A.16. Операционное: переустановка плагина не перерегистрирует скиллы в идущей
сессии.** Реестр читается при старте; после `uninstall`+`install` посреди
работы `mvp:*` перестают резолвиться до новой сессии. На прогон это не влияет —
`workflow.mjs` вызывается по пути, — но `mvp:retro`/`mvp:resume` в такой сессии
недоступны. Не дефект плагина, а свойство загрузчика; фиксируется как известное
ограничение.
