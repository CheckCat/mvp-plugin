#!/usr/bin/env node
// plan-io.mjs — the ONLY reader/writer of .mvp/plan.json at runtime.
//
// Run from the TARGET PROJECT root (not this plugin repo). State lives under
// .mvp/ (plan.json, briefs/, reports/, ledger.md, telemetry/events.jsonl,
...
//        [--arm <cap30|control>]      CAP-experiment arm (спека §9) -> additive
//        [--segments <n>]             event field; both omitted on passive/off runs
...
//   next [--task <id>] [--accept-dirty]  pick the next task / halt
//   ledger --task <id> --sha <sha>    append-only ledger.md ('HEAD' resolves via git)
//     [--concern <text> | --concern-b64 <byteLen>.<base64>]
...
// VALID_EXPERIMENTS: the CAP-experiment mode vocabulary (спека §7-9). Same
// three values and the same "unknown/absent -> greedy" alpha default as
// lib/experiments.sh's mode() — two independent readers of the same
// .mvp/state.json key, so the fallback must match exactly or `next`'s
// `experiments` field and experiments.sh's own mode() could disagree about
// which arm a given task ran in.
const VALID_EXPERIMENTS = ['off', 'passive', 'greedy'];
const STATE_JSON_PATH = path.join(STATE_DIR, 'state.json');
...
// parseFlags(args, allowed, booleans) — `booleans` names the flags that take
// NO value and set `true` by presence. Everything else still REQUIRES a value:
// a flag silently defaulting to true would make `--tokens` with a missing
// argument look like a successful call.
function parseFlags(args, allowed, booleans = []) {
  const positionals = [];
  const flags = {};
...
// are NOT under .mvp (files inside state are allowed to be dirty —
...
// tasksOfSafe(plan) -> tasksOf(plan) or []. Used only to enrich a halt hint:
// a malformed plan must still produce its halt, never an exception from the
// cosmetic branch.
function tasksOfSafe(plan) {
  try {
    const t = tasksOf(plan);
...
function cmdNext(args) {
  const { flags } = parseFlags(args, ['task', 'accept-dirty'], ['accept-dirty']);

  // mech_roles (финальное ревью, находка C1): существуют ли на диске файлы
  // узких ролей механики конвейера (.claude/agents/mvp-*.md). Проверка живёт
  // здесь, а не в workflow.mjs, потому что у оркестратора нет файловой
  // системы; результат едет АДДИТИВНЫМ полем в КАЖДОМ ответе next — включая
  // халты: за all-done следует не-retryable релей установки фазы, и решение
  // «диспатчить ли узкую роль» нужно оркестратору до него. Проект, собранный
  // плагином до появления этих ролей, не имеет ни файлов, ни записей в lock;
  // диспатч несуществующей роли возвращает пустой результат, а повторять
  // не-retryable команду после пустого ответа нельзя — она могла успеть
  // исполниться. Единственный безопасный путь — вообще не диспатчить роль,
  // которой нет; этим полем оркестратор и узнаёт об этом заранее.
  // Лечение для оператора: mvp:sync (собирает роли механики безусловно),
  // затем перезапуск сессии.
  const mechRoleFile = (r) => fs.existsSync(path.join('.claude', 'agents', `${r}.md`));
  const mech_roles = {
    reviewer: mechRoleFile('mvp-reviewer'),
    validator: mechRoleFile('mvp-validator'),
    relay: mechRoleFile('mvp-relay'),
  };
  ...
  if (fs.existsSync(INTERRUPT_PATH)) {
    return finish(true, null, null, { halt: 'interrupt', mech_roles });
  }
  ...
  // --accept-dirty: an OPERATOR-only escape, never set by the workflow itself.
  //
  // The dead end it exists for (measured, trellis task 019): park() leaves the
  // working tree ALONE when the task boundary normalizes to the repo root —
  // there is no safe partial reset there, design note 18. The task is then
  // `failed` with its own work still in the tree, and this dirty check runs
  // BEFORE the --task branch below, so re-dispatching that very task halts on
  // its own uncommitted output. Two individually correct rules, no way out:
  // the operator must either destroy finished work or finalize outside the
  // pipeline. Both happened before this flag existed.
  //
  // Why a flag and not a rule. The obvious rule — "dirt inside the task's
  // boundary is fine" — is exactly wrong for the case that needs it: the
  // boundary IS the repo root there, so the rule would accept any dirty tree
  // whatsoever and the gate would evaporate. A human who looked at the tree
  // is the only thing that can tell "this is task 019's own work" from "this
  // is something else". So the judgement stays with them and is recorded as
  // an argument they typed.
  const dirtyFiles = getDirtyFilesOutsideState();
  if (dirtyFiles.length && !flags['accept-dirty']) {
    const parked = flags.task && tasksOfSafe(plan).find((t) => t.id === flags.task && t.status === 'failed');
    const hint = parked
      ? `task ${flags.task} is failed with an unreset tree — park() does not reset a repo-root boundary. `
        + 'If these files are that task\'s own work, re-run with --accept-dirty; otherwise reset them or finalize outside the pipeline.'
      : null;
    return finish(true, null, null, { halt: 'dirty-tree', files: dirtyFiles, hint, mech_roles });
  }
  ...
  if (flags.task) {
    ...
    if (!chosen) {
      ...
      if (allDone) {
        return finish(true, null, null, { halt: 'all-done', mech_roles });
      }
      const blocking = tasks.filter((t) => t.status !== 'done');
      const detail = blocking.length
        ? `blocking tasks: ${blocking.map((t) => `${t.id}(${t.status})`).join(', ')}`
        : 'no tasks in plan';
      return finish(true, null, null, { halt: 'dag-stuck', detail, mech_roles });
    }
  }

  const briefPath = writeBrief(chosen, byId);

  // experiments (спека §7-9): .mvp/state.json key "experiments", same
  // fallback as lib/experiments.sh's mode() — unreadable/missing state.json
  // or an unrecognized value both fall back to "greedy" (alpha default), read
  // via a local try/catch (state.sh has no "get experiments" verb, and this
  // is a read, not a write — plan-io stays a read-only observer of state.json
  // here, never a second writer of it).
  let experiments = 'greedy';
  try {
    const state = JSON.parse(fs.readFileSync(STATE_JSON_PATH, 'utf8'));
    if (state && VALID_EXPERIMENTS.includes(state.experiments)) experiments = state.experiments;
  } catch {
    // no state.json / unreadable / not JSON -> greedy, same as experiments.sh
  }

  // capped_role: the CAP-arm's dispatch target (Task 9 will start writing
  // `.claude/agents/<role>-capped.md`; until then this is always null and
  // workflow.mjs's CAP-рукав stays dead by construction — see that file's
  // design note for the safe-ordering argument).
  const cappedPath = path.join('.claude', 'agents', `${chosen.role}-capped.md`);
  const capped_role = fs.existsSync(cappedPath) ? `${chosen.role}-capped` : null;
  ...
  finish(true, null, null, {
    ...
    capped_role,
    mech_roles,
    // task_index (финальное ревью, находка I6): позиция задачи в plan.tasks.
    // Стабильна между запусками (add-task только дописывает в конец,
    // существующие задачи не переставляются) — по ней CAP-рукав режет
    // чёт/нечет вместо счётчика задач внутри одного запуска, который
    // обнулялся на каждом старте оркестратора и смещал выборку.
    task_index: tasks.indexOf(chosen),
...
function cmdComplete(args) {
  const { positionals, flags } = parseFlags(args, ['tokens', 'dispatches', 'write-msg', 'arm', 'segments']);
  ...
  // Находка финального ревью (минорная): раньше невалидный --arm молча
  // отбрасывался (`if (flags.arm === 'cap30' || flags.arm === 'control')`
  // ниже просто не срабатывало) — опечатка в вызывающем workflow.mjs тихо
  // роняла задачу из выборки эксперимента H1 без единого сигнала. Отказ
  // явный, до любой мутации плана/телеметрии.
  if (flags.arm !== undefined && flags.arm !== 'cap30' && flags.arm !== 'control') {
    return finish(false, `--arm must be 'cap30' or 'control', got '${flags.arm}'`, 'usage: plan-io.mjs complete <id> --tokens <n> [--arm cap30|control] [--segments <n>]', null);
  }
  // Второй проход финального ревью: то же поведение для --segments — раньше
  // невалидное число (Number(...) -> NaN) молча отбрасывалось, событие
  // писалось без поля segments, и h1-cap.sh терял сегменты задачи из
  // mean_segments без единого сигнала. Отказ явный, до любой мутации.
  const segsFlag = flags.segments === undefined ? null : Number(flags.segments);
  if (flags.segments !== undefined && !Number.isFinite(segsFlag)) {
    return finish(false, `--segments must be a number, got '${flags.segments}'`, 'usage: plan-io.mjs complete <id> --tokens <n> [--arm cap30|control] [--segments <n>]', null);
  }
  ...
  // keep reading `delta_tokens`.
  //
  // What `delta_tokens` IS (measured 2026-09-21, see
  // docs/observations/2026-09-21-delta-tokens-measures-output.md): the
  // OUTPUT tokens of the whole run — controller plus every subagent. Over
  // one 110-agent run it came to 783 887 against the journal's own 800 128
  // output tokens for those agents: a 2% gap, i.e. the same quantity.
  //
  // What it is NOT: cost. Output is ~14% of the real spend; the mass is
  // cache creation (3 995 364 vs 800 128 on that run). The earlier claim
  // here — "controller only, understates true cost by ~8.4x" — compared
  // output-only against a full-spend figure and read the ratio as a loss of
  // subagent usage. `controller_only: true` said so in the data itself and
  // is no longer written; events predating this fix still carry it.
  //
  // `dispatches` remains the best available cost proxy, but for a different
  // reason than was recorded: it tracks cache creation, which is where the
  // spend actually is. The run's true total is only knowable outside this
  // script — `subagent_tokens` in the Workflow completion notification.
  ...
  // arm/segments: additive CAP-experiment fields (спека §9), consumer —
  // scripts/experiments/h1-cap.sh. Written ONLY when the caller passed
  // --arm/--segments (workflow.mjs's greedy CAP-рукав) — a plain event with
  // neither field is legal and expected on every passive/off run, and every
  // control-arm task within a greedy run.
  if (flags.arm === 'cap30' || flags.arm === 'control') event.arm = flags.arm;
  // segsFlag провалидирован выше, до мутаций: сюда доходит либо null (флага
  // не было), либо конечное число.
  if (segsFlag !== null) event.segments = segsFlag;
...
// decodeConcernPayload(raw) -> { text } | { error }
//
// Accepts the `<byteLen>.<base64>` payload that workflow.mjs builds for
// --concern-b64, and refuses anything that does not decode back to exactly
// what was encoded.
//
// Why this exists (task 018 of the trellis run, 2026-09-17): concerns used to
// cross into this script as free prose in argv, shell-quoted by workflow.mjs.
// The chain `complete && finalize.sh && ledger` is executed by a RELAY AGENT,
// not by a shell the workflow controls, and the agent is asked to reproduce
// the command verbatim. On a concern block that was multi-line and carried
// embedded JSON with escaped quotes, the agent did not reproduce it — it
// re-authored the command, closed the --concern quote early and invented a
// flag that does not exist:
//     ... --concern 'CONCERN 1: ...' --findings '[{"severity":"bug",...}]'
// -> {"ok":false,"reason":"unknown flag: --findings"}
// By then `complete` and `finalize.sh` had already run: the task was marked
// done and COMMITTED, and only the audit trail was lost.
//
// A single opaque ASCII token gives the relay nothing to improve: no quotes,
// no newlines, no prose, no non-ASCII. The length prefix is what turns the
// remaining failure mode from silent into loud — a relay that truncates the
// blob would otherwise decode to plausible-looking shorter text and write a
// quietly mutilated ledger. Canonical re-encoding catches substitution the
// same way. Both checks are cheap; a corrupted audit trail is not.
function decodeConcernPayload(raw) {
  const value = String(raw);
  const dot = value.indexOf('.');
  if (dot <= 0) {
    return { error: 'concern payload must be "<byteLen>.<base64>"' };
  }
  const declared = Number(value.slice(0, dot));
  const b64 = value.slice(dot + 1);
  if (!Number.isInteger(declared) || declared < 0) {
    return { error: 'concern payload length prefix is not a non-negative integer' };
  }
  if (!/^[A-Za-z0-9+/]*={0,2}$/.test(b64)) {
    return { error: 'concern payload is not base64' };
  }
  const buf = Buffer.from(b64, 'base64');
  if (buf.length !== declared) {
    return { error: `concern payload truncated in transit: declared ${declared} bytes, decoded ${buf.length}` };
  }
  if (buf.toString('base64') !== b64) {
    return { error: 'concern payload altered in transit: base64 is not canonical' };
  }
  return { text: buf.toString('utf8') };
}

function cmdLedger(args) {
  const { flags } = parseFlags(args, ['task', 'sha', 'concern', 'concern-b64']);
  if (!flags.task || !flags.sha) {
    return finish(false, 'missing --task/--sha', 'usage: plan-io.mjs ledger --task <id> --sha <sha> [--concern <text> | --concern-b64 <byteLen>.<base64>]', null);
  }

  // --concern stays for operators driving this script by hand (recovery after
  // a failed run); --concern-b64 is what the pipeline uses. Passing both is a
  // caller bug, not something to silently resolve by precedence.
  let concernText = flags.concern ? String(flags.concern) : null;
  if (flags['concern-b64'] !== undefined) {
    if (concernText !== null) {
      return finish(false, 'both --concern and --concern-b64 given', 'pass exactly one', null);
    }
    const decoded = decodeConcernPayload(flags['concern-b64']);
    if (decoded.error) {
      return finish(false, decoded.error, 'workflow.mjs builds this payload; a mismatch means the relay altered the command', null);
    }
    concernText = decoded.text;
  }
